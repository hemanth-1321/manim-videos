    ...
    <div className="w-full max-w-6xl mx-auto p-4">
      <Tabs defaultValue="all" className="w-full">
        ...
        <TabsContent value="user">
          {token ? <VideoGrid videos={userVideos} /> : <p className="text-center text-gray-500">Login to view your videos.</p>}
        </TabsContent>
      </Tabs>
    </div>
  );
    ...
    return (
      <div className="columns-1 sm:columns-2 md:columns-3 gap-4 space-y-4">
        {videos.map((video) => (
          <div
            key={video.id}
            className="relative group overflow-hidden rounded-xl shadow-md break-inside-avoid"
          >
            <video
              src={video.url}
            ...
            >
              <Download className="cursor-pointer" size={24} />
            </button>
