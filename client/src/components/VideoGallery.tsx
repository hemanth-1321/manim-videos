...
import { useAuthStore } from '@/stores/useAuth'; // import the Zustand store
...
export default function VideoGallery() {
  ...
  // Sync token to the Zustand store
  useEffect(() => {
    const storedToken = typeof window !== 'undefined' ? localStorage.getItem('token') : null;
    if (storedToken) setToken(storedToken); // Set token on mount
    fetchAllVideos();
  }, [setToken]);
  ...
  const fetchAllVideos = async () => {
    try {
      ...
      const rawVideos = response.data?.response || [];
    ...
    <div className="w-full max-w-6xl mx-auto p-4">
      <Tabs defaultValue="all" className="w-full">
        ...
        < TabsContent value="user">
          {token ? <VideoGrid videos={userVideos} /> : <p className="text-center font-bold">Login to view your videos.</p>}
        </TabsContent>
      </Tabs>
    </div>
  );
...
function VideoGrid({ videos }: { videos: Video[] }) {
  ...
  const handleVideoClick = (videoId: string) => {
    setShowDownload((prevState) => (prevState === videoId ? null : videoId));
  };
    ...
    <div className="columns-1 sm:columns-2 md:columns-3 gap-4 space-y-4">
     ...
     >
          ...
          {showDownload === video.id && (
            <button
              onClick={() => handleDownload(video.url)}
              className="absolute top-4 right-4 p-2 bg-opacity-50 rounded-full shadow-md sm:opacity-100 sm:group-hover:opacity-100 opacity-100"
            >
              <Download className="cursor-pointer text-white" size={24} />
            </button>
          )}
